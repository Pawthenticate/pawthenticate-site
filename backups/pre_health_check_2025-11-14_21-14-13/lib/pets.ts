...
function petDataToInsert(petData: Partial<PetData>, userId: string): any {
  return {
    ...
    // Document fields (stored as JSONB)
 ...
 */
function rowToPetData(row: PetRow): PetData & { id: string; createdAt: string; updatedAt: string } {
  return {
    // Metadata
...
export async function updatePet(
  ...
  try {
    ...
    const fullUpdateData = {
      ...
      ...(petData.preferredTemplate
        ? { preferred_template: petData.preferredTemplate }
        : {}),
      // Add flea/worm treatment status (missing from generated types)
      flea_worm_treatment_status: petData.fleaWormTreatmentStatus || null,
      // Document fields (JSONB)
      vaccination_certificate: petData.vaccinationCertificate || null,
      desexing_certificate: petData.desexingCertificate || null,
